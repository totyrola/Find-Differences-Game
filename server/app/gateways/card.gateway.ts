...
export default class CardGateway {
    ...
    @SubscribeMessage(Events.ToServer.RESET_BEST_TIMES)
    async resetBestTimesByCardId(client: Socket, cardId: string) {
        this.mongoDBService.resetBestTimesByCardId(cardId).then((newTimes) => {
            newTimes.id = cardId;
            OutputFilterGateway.sendCardTimes.toServer(newTimes);
        });
    }

    @SubscribeMessage(Events.ToServer.RESET_ALL_BEST_TIMES)
    resetAllBestTimes() {
        this.mongoDBService.resetAllBestTimes().then((newTimes) => {
            OutputFilterGateway.sendAllCardPreviewTimes.toServer(newTimes);
        });
    }
