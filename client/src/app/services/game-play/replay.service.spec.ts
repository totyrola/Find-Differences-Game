...
describe('ReplayService', () => {
    ...
    beforeEach(() => {
        ...
        delayServiceSpy = jasmine.createSpyObj(
            'DelayService',
            ['changeSpeed', 'doCyclicly', 'wait', 'pauseTime', 'resumeTime', 'clearDelays', 'clearCycles'],
            { timeIsPaused: false },
        );
    ...
    it('doAndStore should execute action and store it', () => {
        ...
        service.doAndStore(test);
        expect(storeSpy).toHaveBeenCalledWith(test);
    ...
    it('store should add the replayAction to the list if it is not currently replaying', () => {
        ...
        expect(service['replayActions'][0].action).toEqual(test);
    });
    ...
    it('should set the correct values for replay', () => {
        delayServiceSpy.timeIsPaused = true;
        gameTimeServiceSpy.isReplayMode = false;
        gameTimeServiceSpy.recordedTimeIndex = 5;
        service.isReplayMode = false;
        service.isReplaying = false;
        service['actionIndex'] = 3;
        service['replay']();
        expect(delayServiceSpy.timeIsPaused).toEqual(false);
        expect(gameTimeServiceSpy.isReplayMode).toEqual(true);
        expect(gameTimeServiceSpy.recordedTimeIndex).toEqual(0);
        expect(service.isReplayMode).toEqual(true);
        expect(service.isReplaying).toEqual(true);
        expect(service['actionIndex']).toEqual(0);
    });

    it('should create a cyclic timer when replaying', () => {
        service['replay']();
        expect(delayServiceSpy.doCyclicly).toHaveBeenCalled();
    });
    ...
    it('isSameTime should compare times with precision of 1 decimal', () => {
        const time1 = 1.16;
        const time2 = 1.18;
        const time3 = 1.35;
        expect(service['isSameTime'](time1, time2)).toBeTruthy();
        expect(service['isSameTime'](time1, time3)).toBeFalse();
    });

    it('round should round with a precision of 1 decimal', () => {
        const number = 3.567;
        const number1Decimal = 36;
        expect(service['round'](number)).toEqual(number1Decimal);
    });
});
