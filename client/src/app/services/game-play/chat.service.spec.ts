...
describe('ChatService', () => {
    ...
    it('should receive a chat message from the socket service', () => {
        spyOn<any>(service, 'getCurrentTime').and.returnValue('2023-04-15T10:00:00.000Z');
        spyOn(service, 'pushMessage' as never).and.callThrough();
        const messageDto: ChatMessageOutputDto = { sender: 'John', message: 'Hello world' };
        const message: Message = {
            text: `${messageDto.sender}: ${messageDto.message}`,
            time: '2023-04-15T10:00:00.000Z',
            isMessageText: true,
            messageReceived: true,
        };

        socketService.peerSideEmit(FromServer.CHAT_MESSAGE, messageDto);
        expect(service.messages).toContain(message);
    ...
    it('should add global message to messages when receiving global_message event', () => {
        ...
        const text = ' - ';
        const message: Message = {
            text: '2023-04-15T10:00:00.000Z - ' + text,
            time: '2023-04-15T10:00:00.000Z',
            isMessageText: false,
            messageReceived: false,
        };
    ...
    it('should send a chat message to the server if isMessage is true', () => {
        ...
        expect(socketService.send).toHaveBeenCalledWith(ToServer.CHAT_MESSAGE, { gameId: '123', message: validInput });
    });
